...
void main() {
    ...
    float glowIntensity = 0.25; // Adjust this value for more or less glow

    // Apply variable color for each hexagon based on time and add glow
    if (d1 < 0.0) color += vec3(sin(time), cos(time), sin(time * 12.0)) * smoothstep(0.0, glowIntensity, -d1);
    if (d2 < 0.0) color += vec3(cos(time), sin(time * 3.0), cos(time)) * smoothstep(0.0, glowIntensity, -d2);
    if (d3 < 0.0) color += vec3(sin(time * 5.0), cos(time), sin(time)) * smoothstep(0.0, glowIntensity, -d3);
    if (d4 < 0.0) color += vec3(cos(time * 7.0), sin(time), cos(time * 7.0)) * smoothstep(0.0, glowIntensity, -d4);
    if (d5 < 0.0) color += vec3(sin(time), cos(time * 9.0), sin(time * 9.0)) * smoothstep(0.0, glowIntensity, -d5);
